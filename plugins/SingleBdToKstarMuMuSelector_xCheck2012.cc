#define SingleBdToKstarMuMuSelector_xCheck2012_cxx
// The class definition in SingleBdToKstarMuMuSelector_xCheck2012.h has been generated automatically
...
//
// Root > T->Process("SingleBdToKstarMuMuSelector_xCheck2012.C")
// Root > T->Process("SingleBdToKstarMuMuSelector_xCheck2012.C","some options")
// Root > T->Process("SingleBdToKstarMuMuSelector_xCheck2012.C+")
//
...
#include <sstream>
#include <map>
#include "SingleBdToKstarMuMuSelector_xCheck2012.h"
#include <TH2.h>
#include <TStyle.h>
#include <TProof.h>
...
void SingleBdToKstarMuMuSelector_xCheck2012::Begin(TTree * /*tree*/)
{//{{{
...
void SingleBdToKstarMuMuSelector_xCheck2012::SlaveBegin(TTree * /*tree*/)
{//{{{
...
Bool_t SingleBdToKstarMuMuSelector_xCheck2012::Process(Long64_t entry)
{//{{{
   // The Process() function is called for each entry in the tree (or possibly
   // keyed object in the case of PROOF) to be processed. The entry argument
   // specifies which entry in the currently loaded tree is to be processed.
   // It can be passed to either SingleBdToKstarMuMuSelector_xCheck2012::GetEntry() or TBranch::GetEntry()
...
void SingleBdToKstarMuMuSelector_xCheck2012::SlaveTerminate()
{//{{{
...
void SingleBdToKstarMuMuSelector_xCheck2012::Terminate()
{//{{{
...
int SingleBdToKstarMuMuSelector_xCheck2012::SelectB(string cut)
{//{{{
    // To be fixed
...
bool SingleBdToKstarMuMuSelector_xCheck2012::HasGoodDimuon(int i)
{//{{{
    // To be fixed
    return true; 
}//}}}

void SingleBdToKstarMuMuSelector_xCheck2012::SaveEvent(int i)
{//{{{
    ...
    // In 2012 analysis, both mass hypothesis are computed.
    if (B0notB0bar){
...
void SingleBdToKstarMuMuSelector_xCheck2012::SaveGen()
{//{{{
...
int main(int argc, char** argv) {
    ...
    TChain *ch = new TChain("B0KstMuMu/B0KstMuMuNTuple"); 
    ch->Add(infile.Data()); 
    ...
    ch->Process("SingleBdToKstarMuMuSelector_xCheck2012.cc+", option, nentries, iStart); 
